...
if (!BOT_TOKEN || !DOMAIN) {
  console.error("ENV belum lengkap. BOT_TOKEN / WEBHOOK_DOMAIN kosong.");
  process.exit(1);
}
...
const logPost = (ctx, gender, content) => {
...
bot.on(["text", "photo", "video"], async (ctx) => {
  ...
  const gender = text.includes("#pria")
    ? "Pria"
    : text.includes("#wanita")
    ? "Wanita"
    : null;
  ...
  if (ctx.message.text) {
    ...
    logPost(ctx, gender, ctx.message.text);
  }
...
bot.command("ban", (ctx) => {
  ...
  const [, id, jam] = ctx.message.text.split(" ");
...
(async () => {
  ...
  app.listen(PORT, () => {
    ...
    console.log("Webhook:", `${DOMAIN}/telegram`);
  });
})();
